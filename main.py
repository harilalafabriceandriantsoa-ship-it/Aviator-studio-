...
# CSS nohamafisina ho an'ny Placeholder sy ny Styles
st.markdown("""
<style>
...
/* PLACEHOLDER NOHAFESINA: Mainty kokoa sy hita tsara */
...
.stTextInput input{background:rgba(255,255,255,.15)!important;border:2px solid rgba(255,0,102,.5)!important;color:#fff!important;border-radius:11px!important;font-size:.93rem!important;padding:11px 14px!important}
.stNumberInput input{background:rgba(255,255,255,.15)!important;border:2px solid rgba(255,0,102,.5)!important;color:#fff!important;border-radius:11px!important}
</style>
""", unsafe_allow_html=True)

# Function handikana ny data ho any amin'ny press-papier
...
with ci:
    ...
    ti=st.text_input("⏰ TIME ROUND PRÉCÉDENT",placeholder="20:22:24")
    ...
    if st.button("🚀 ANALYSER",use_container_width=True):
        if h5 and ti:
            ...
            sj(HF,st.session_state.H); st.session_state.ck+=1
            
            # Adika hoazy ny Hash, Cote, sy Time
            data_to_copy = f"Hash: {h5.strip()} | Cote: {lc} | Time: {ti.strip()}"
            copy_to_clipboard(data_to_copy)
            st.rerun()
